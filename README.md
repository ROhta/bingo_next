...
他リポジトリ共通の指示 (PR レビュー方針・APM 運用ルール・使用言語・開発フロー等) は共通パッケージ [`ROhta/apm-config`](https://github.com/ROhta/apm-config) から `apm install` で配信され、ローカルの `.apm/instructions/` には保持しません。共通指示を変更したい場合は apm-config を編集します。
